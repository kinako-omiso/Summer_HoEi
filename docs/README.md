...
マップ生成時は候補Sceneを最初に1回ずつ選び、6部屋に足りない残り枠だけを復元抽選します。現在の`room_a`、`room_b`、`room_c`は各マップへ最低1部屋ずつ配置され、残り3部屋では同じSceneが複数回選ばれる場合があります。候補が6種類を超えた場合は、候補をシャッフルして重複なしで6種類を選びます。候補を増やすために生成コードを編集する必要はありません。

ライトなしの天井が選ばれた部屋と廊下では、壁と床へ弱い自己発光を設定して輪郭を確認できるようにします。ライト付き部屋がライトなし廊下へ接続する場合は、廊下側から見える入口壁も自己発光します。それ以外のライト付き区画には自己発光を追加しません。自己発光は周囲を照らす光源ではなく、壁や床自体の最低限の視認性を保つためのものです。

壁と床の自己発光は通常の部屋・廊下ライトと同じくブレーカーへ接続されており、ブレーカーを落とすと消灯します。

廊下の壁・床・天井の表示形状は、接続する部屋の壁面間に収まります。ナビゲーションの継ぎ目を防ぐための余長は床のコリジョンだけに設定し、表示形状は部屋内へ入り込みません。

スタート側エレベーターの天井ライトは、ライト付き天井と同じ光量・範囲・減衰でエレベーター内を照らします。このライトもブレーカーによる停電対象です。ゴール側エレベーターには追加されません。

マップを生成するたびに、保存済みの部屋Sceneとその依存リソースを再読み込みします。入口になる壁だけがドア付き壁または開放入口へ置き換えられ、入口にならない壁、床、天井、家具、ライトなどの保存内容は維持されます。ランダム候補グループの家具はカテゴリごとに0個から保存数まで抽選し、全カテゴリが0個になった場合だけランダムな候補を1個残します。各部屋Sceneにブレーカーが含まれていても、生成後のマップには抽選された1台だけが残ります。残るブレーカーは、設置壁が入口になる場合を除いてSceneのTransformを維持します。
