...
マップを生成するたびに、保存済みの部屋Sceneとその依存リソースを再読み込みします。入口になる壁だけがドア付き壁または開放入口へ置き換えられ、入口にならない壁、床、天井、家具、ライトなどの保存内容は維持されます。ランダム候補グループの家具は、保存された配置・設定を使ったうえで従来どおり個数が抽選されます。ブレーカーも設置壁が入口になる場合を除いてSceneのTransformを維持します。
