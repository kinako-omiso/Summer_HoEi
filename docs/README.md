...
マップを生成するたびに、保存済みの部屋Sceneとその依存リソースを再読み込みします。入口になる壁だけがドア付き壁または開放入口へ置き換えられ、入口にならない壁、床、天井、家具、ライトなどの保存内容は維持されます。ランダム候補グループの家具は、保存された配置・設定を使ったうえで従来どおり個数が抽選されます。各部屋Sceneにブレーカーが含まれていても、生成後のマップには抽選された1台だけが残ります。残るブレーカーは、設置壁が入口になる場合を除いてSceneのTransformを維持します。
...
- ブレーカーは任意。ルートを`map_breaker`グループへ登録すると生成候補になり、マップ全体で抽選された1台だけが残る。設置壁が入口になる場合は、入口のない壁へ自動移動する。
- ランダム削減する家具には、既存部屋と同じ候補グループを設定する。
- 家具などには実形状に合う`CollisionShape3D`を設定する。入口のドア抽選時、このコリジョンがドアの開閉範囲と重なる場合はドアが追加されず、開放入口になる。
